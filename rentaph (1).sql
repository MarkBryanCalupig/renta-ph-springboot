...
select * from property p join landlord l on p.landlord_id=l.id where p.id = 1;

select * from landlord l join property p on p.landlord_id=l.id where l.id=1 and p.availability=0;

select sum(price) from landlord l join property p on p.landlord_id=l.id where l.id=1 and p.availability=0;

select count(price) from landlord l join property p on p.landlord_id=l.id where l.id=1 and p.availability=0;

select availability from property where id=1;
update (select availability from property where id=1)
set availability=1;


update (select quantity 
from orderdetails od join products p ON od.productid = p.productid
where orderid = 10250 and p.productname like 'Jack''s New England Clam Chowder')
set quantity = 99;